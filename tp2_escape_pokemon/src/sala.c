...
*/
char *concatenar_strings(char *nombre_interaccion, char *string1, char *string2,
			 char *string3)
{
...
*/
hash_t *cargar_elementos(sala_t *sala, const char *nombre_archivo,
			 char tipo_elemento)
{
	...
	if (tipo_elemento == OBJETOS) {
		struct objeto *objeto_a_agregar =
			objeto_crear_desde_string(linea);
		hash_insertar(sala->jugador->objetos_conocidos,
			      objeto_a_agregar->nombre, objeto_a_agregar);
	...
	while (linea_leida) {
		if (tipo_elemento == OBJETOS) {
			struct objeto *objeto_a_agregar =
				objeto_crear_desde_string(linea);
			hash_insertar(sala->objetos, objeto_a_agregar->nombre,
				      objeto_a_agregar);
			hash = sala->objetos;
			sala->cantidad_objetos++;
		} else if (tipo_elemento == INTERACCIONES) {
			struct interaccion *interaccion_a_agregar =
				interaccion_crear_desde_string(linea);

			char nombre_interaccion[MAX_NOMBRE_INTERACCION] = "";
			concatenar_strings(
				nombre_interaccion,
				interaccion_a_agregar->objeto,
				interaccion_a_agregar->verbo,
				interaccion_a_agregar->objeto_parametro);
			hash_insertar(sala->interacciones, nombre_interaccion,
				      interaccion_a_agregar);
				  ...
				  const char *interacciones)
{
	...
	if (cargar_elementos(sala, interacciones, INTERACCIONES))
		sala->cantidad_interacciones =
			hash_cantidad(sala->interacciones);
	else {
		sala_destruir(sala);
		return NULL;
	}
...
char **obtener_nombres_objetos(hash_t *hash, int *cantidad)
{
	...
	char **nombres_objetos =
		malloc((unsigned)hash_cantidad(hash) * sizeof(char *));
...
char **sala_obtener_nombre_objetos_conocidos(sala_t *sala, int *cantidad)
{
	...
	return obtener_nombres_objetos(sala->jugador->objetos_conocidos,
				       cantidad);
}
...
char **sala_obtener_nombre_objetos_poseidos(sala_t *sala, int *cantidad)
{
	...
	return obtener_nombres_objetos(sala->jugador->objetos_poseidos,
				       cantidad);
}
...
bool sala_agarrar_objeto(sala_t *sala, const char *nombre_objeto)
{
	...
	struct objeto *objeto_actual =
		hash_obtener(hash_objetos_conocidos, nombre_objeto);
	if (objeto_actual == NULL)
		return false;

	if (objeto_actual->es_asible) {
		hash_quitar(hash_objetos_conocidos, nombre_objeto);
		hash_insertar(sala->jugador->objetos_poseidos, nombre_objeto,
			      objeto_actual);
		return true;
	}
...
char *sala_describir_objeto(sala_t *sala, const char *nombre_objeto)
{
	...
	if (objeto_actual == NULL)
		objeto_actual =
			hash_obtener(hash_objetos_poseidos, nombre_objeto);
	if (objeto_actual == NULL)
		return NULL;
...
bool son_objetos_validos(sala_t *sala, const char *objeto1, const char *objeto2)
{
	...
	if (objeto1_actual != NULL) {
		...
		objeto1_actual =
			hash_obtener(sala->jugador->objetos_conocidos, objeto1);
		if (objeto1_actual != NULL) {
	...
	if (strcmp(objeto2, "") != 0) {
		objeto2_actual =
			hash_obtener(sala->jugador->objetos_conocidos, objeto2);
	...
	} else if (es_objeto_valido && hay_objeto_parametro &&
		   es_objeto_parametro_valido) {
		es_interaccion_valida = true;
	}
...
void descubrir_objeto(sala_t *sala, struct interaccion *interaccion_actual)
{
	...
	hash_insertar(sala->jugador->objetos_conocidos,
		      interaccion_actual->accion.objeto, objeto_actual);
	sala->cantidad_objetos--;
	sala->jugador->cantidad_objetos_conocidos++;
}
...
*/
void reemplazar_objeto(sala_t *sala, struct interaccion *interaccion_actual,
		       bool hay_objeto_parametro)
{
	void *objeto_reemplazo =
		hash_quitar(sala->objetos, interaccion_actual->accion.objeto);
	hash_insertar(sala->jugador->objetos_conocidos,
		      interaccion_actual->accion.objeto, objeto_reemplazo);

	if (hay_objeto_parametro) {
		void *objeto_quitado =
			hash_quitar(sala->jugador->objetos_conocidos,
				    interaccion_actual->objeto_parametro);
		free(objeto_quitado);
		sala->jugador->cantidad_objetos_conocidos--;
	}
	sala->cantidad_objetos--;
...
{
	void *quitado = hash_quitar(sala->jugador->objetos_poseidos,
				    interaccion_actual->accion.objeto);
			      ...
			      void *aux)
{
	...
	char nombre_interaccion[MAX_NOMBRE_INTERACCION] = "";
	concatenar_strings(nombre_interaccion, (char *)objeto1, (char *)verbo,
			   (char *)objeto2);
	struct interaccion *interaccion_actual =
		hash_obtener(hash_interacciones, nombre_interaccion);
	...
	while (interaccion_actual != NULL) {
		...
		case DESCUBRIR_OBJETO:
			...
			mostrar_mensaje(interaccion_actual->accion.mensaje,
					interaccion_actual->accion.tipo, aux);
			interacciones_ejecutadas++;
			break;

		case REEMPLAZAR_OBJETO:;

			reemplazar_objeto(sala, interaccion_actual,
					  hay_objeto_parametro);
			mostrar_mensaje(interaccion_actual->accion.mensaje,
					interaccion_actual->accion.tipo, aux);
			interacciones_ejecutadas++;
			break;
				...
				const char *objeto1, const char *objeto2)
{
	...
	struct interaccion *interaccion =
		hash_obtener(sala->interacciones, nombre_interaccion);
	if (interaccion == NULL)
		return false;
