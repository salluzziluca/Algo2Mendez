...
void hash_insertar_inserta_correctamente(){
	...
	pa2m_afirmar(hash_contiene(hash, "tres"), "el hash contiene esa clave");
	...
	hash = hash_crear(3);
	anterior = NULL;
	pa2m_afirmar(hash_insertar(hash, "uno",&uno , &anterior) != NULL, "Se inserta <uno,1>");
	pa2m_afirmar(hash_insertar(hash, "uno", &uno, &anterior) != NULL, "Se inserta <uno,1>");
	pa2m_afirmar(*(int *)anterior == uno, "Se puede insertar dos veces el mismo par, genera que se sobreescriba el valor por el mismo que fue quitado");
...
void hash_con_cada_clave_itera_correctamente(){
	...
	pa2m_afirmar(hash_con_cada_clave(hash, NULL, aux) == 0, "No se puede iterar una funcion nula");
	hash_destruir(hash);

	hash = hash_crear(10);
	int tres = 3, cuatro = 4, cinco = 5, seis = 6, siete = 7, ocho = 8, nueve = 9;
	...
	hash_insertar(hash, "diez", NULL, &anterior);
	int contador = 0;
